...
public class PrimeCustomer extends Customer implements Discount{


    public PrimeCustomer(String name, int age, String phonenumber, String email_ID, String password) {
    ...
    public double deliverycharges(double totalprice){
        ...
        System.out.println(100+ 0.02*totalprice);
        return 100+ 0.02*totalprice;
    }
    ...
    public double member_specific_discount(double product_price){
        //System.out.println("Member specific discount: "+0.05*product_price);
        return 0.05*product_price;
    }
    ...
    public double use_coupon(double product_price){
        if (this.Coupons.size()>0){
            ...
            return (double)((((double)this.Coupons.get(0))*product_price)/100);
    ...
    public double admin_set_discount(double proID, HashMap<Integer, ArrayList<Integer>> alldiscounts,ArrayList<Category> categories) {
        if (alldiscounts.containsKey(proID)) {
            //System.out.println("Admin set discount: " +(double) alldiscounts.get(proID).get(2));
        ...
        for (Category category : categories) {
            for (Product product : category.products.keySet()) {
                if (product.proID == proID) {
                    if(alldiscounts.get(proID)!=null)return  product.price *(double) (((double) (alldiscounts.get(proID).get(1))) / 100);
                }
            }
    ...
    public void checkout(HashMap<Integer, ArrayList<Integer>> alldiscounts,ArrayList<Category> categories, HashMap <Giweaway, ArrayList<Integer>> allgiweaways){
        ...
        for(Giweaway g: this.giveawaycart.keySet()){
            System.out.println("Price is"+ allgiweaways.get(g).get(1));
            cartprice.add(Double.valueOf(allgiweaways.get(g).get(1)));
            undiscountedprice+=Double.valueOf(allgiweaways.get(g).get(1));
